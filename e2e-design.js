...
;(async () => {
  ...
  const info = await page.evaluate(() => {
    const rows = Array.from(document.querySelectorAll('.g6-rel-row'))
    return {
      rowChipCounts: rows.map((r) => r.querySelectorAll('.g6-rel').length),
      viewportWidth: document.querySelector('.g6-relations-viewport')?.clientWidth,
    }
  })
  console.log(JSON.stringify(info))
  await page.screenshot({ path: path.join(__dirname, 'pack-check.png'), clip: { x: 0, y: 240, width: 1280, height: 420 } })
  await browser.close()
