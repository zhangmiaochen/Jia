...
function closeMenu() { if (props.menuOpen) emit('update:menuOpen', false) }
function onKeydown(event: KeyboardEvent) { if (event.key === 'Escape') closeMenu() }
function isSmallScreen() { return window.matchMedia('(max-width: 700px)').matches }
function syncBodyLock() { document.body.style.overflow = props.menuOpen && isSmallScreen() ? 'hidden' : '' }
function onResize() { if (props.menuOpen && !isSmallScreen()) emit('update:menuOpen', false); else syncBodyLock() }
watch(() => props.menuOpen, syncBodyLock)
onMounted(() => { window.addEventListener('keydown', onKeydown); window.addEventListener('resize', onResize) })
onBeforeUnmount(() => { window.removeEventListener('keydown', onKeydown); window.removeEventListener('resize', onResize); document.body.style.overflow = '' })
</script>
...
<template>
  <div class="app-shell">
    ...
    <main class="main">
      <header class="topbar"><button class="mobile-menu" type="button" aria-label="打开导航菜单" :aria-expanded="menuOpen" @click="emit('update:menuOpen', !menuOpen)">☰</button><div class="family-select"><label>CURRENT FAMILY</label><select :value="activeFamily?.id" @change="emit('familyChange', ($event.target as HTMLSelectElement).value)"><option v-for="family in families" :key="family.id" :value="family.id">{{ family.name }}</option></select><button class="icon-action" title="创建家族" @click="emit('createFamily')">＋</button></div><div class="top-actions"><button class="icon-action" title="通知">♧</button><button class="icon-action" title="退出" @click="emit('logout')">↪</button></div></header>
      <slot />
    </main>
